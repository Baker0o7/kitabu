...
class AiAssistantActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        actions.forEach { (btn, prompt) -> btn.setOnClickListener { ask(prompt) } }
    ...
    private fun ask(prompt: String) {
        val apiKey = SecurePrefsHelper.getAIApiKey(this)
        ...
        if (apiKey.isBlank()) {
            appendBubble(
                sender = "Kitabu AI",
                text   = "No API key set.\n\nGo to Settings and paste your free Google AI Studio key.\n\nGet one at aistudio.google.com - no credit card needed.",
                isUser = false
            )
    ...
    private fun buildSystemPrompt() = buildString {
        ...
        append("When suggesting tags, use #hashtag format with lowercase. ")
        append("When creating outlines, use proper Markdown headers (##, ###). ")
        if (noteContent.isNotBlank()) {
            append("\n\nThe user is currently editing this note - use it as context:\n\n")
            append("---\n")
            append(noteContent.take(8000))
            append("\n---")
        }
    }
    ...
    private fun appendBubble(sender: String, text: String, isUser: Boolean) {
        ...
        val prefix = if (isUser) "> You" else "Kitabu AI"
