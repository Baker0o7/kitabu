...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupDrawer() {
        ...
        binding.navView.setNavigationItemSelectedListener { item ->
            ...
            when (item.itemId) {
                R.id.nav_all_notes   -> { vm.filterByTag(null); vm.showDailyNotes(false); true }
                R.id.nav_daily       -> { vm.showDailyNotes(true); true }
    ...
    private fun observeNotes() {
        vm.notes.observe(this) { list ->
            ...
            supportActionBar?.subtitle      = "${list.size} note${if (list.size != 1) "s" else ""}"
        }
    }
    ...
    private fun confirmDelete(note: Note) {
        MaterialAlertDialogBuilder(this, R.style.KitabuDialog)
            .setTitle("Delete?").setMessage("This cannot be undone.")
