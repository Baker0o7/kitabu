...
object ThemeManager {
    ...
    /** Call this at the top of every Activity's onCreate(), before super.onCreate(). */
    fun apply(context: Context) {
        val activity = context as? android.app.Activity ?: return
        val theme = get(activity)
        val lightSuffix = if (isLightMode(activity)) ".Light" else ""
        try {
            // theme.id is lowercase (e.g. "noir") but XML resources use PascalCase (e.g. "Noir")
            val resName = theme.name.lowercase().replaceFirstChar { it.uppercase() }
            val styleId = activity.resources.getIdentifier(
                "Theme.Kitabu.$resName$lightSuffix", "style", activity.packageName
            )
